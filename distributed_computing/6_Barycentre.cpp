void compute(int total_count, int my_count, float my_points[][3]) {
  ...
  float barycentre[3] = {0.0f, 0.0f, 0.0f};
  ...
  barycentre[0] = barycentre[0]/total_count;
  barycentre[1] = barycentre[1]/total_count;
  barycentre[2] = barycentre[2]/total_count;
  ...
  for (int i=0; i < my_count; ++i) {
    ...
    float x = pow(my_points[i][0] - barycentre[0], 2);
    float y = pow(my_points[i][1] - barycentre[1], 2);
    float z = pow(my_points[i][2] - barycentre[2], 2);
