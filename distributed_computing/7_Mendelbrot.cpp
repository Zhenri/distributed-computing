...
int main(int argc, char **argv) {
  ...
  {  
    ...
    MPI_Scatter(&points, p_count/size, MPI_DOUBLE,
                &points_recv, p_count/size, MPI_DOUBLE, 0,
                MPI_COMM_WORLD);
  }
  MPI_Barrier(MPI_COMM_WORLD);
  ...
  compute_mandelbrot(points_recv, npts, mset);
  MPI_Barrier(MPI_COMM_WORLD);

  if (rank == 0)
  {
    int mset_r[npts*size]; // 
    MPI_Gather(&mset, npts, MPI_INT,
               &mset_r, npts*rank, MPI_INT, 0,
               MPI_COMM_WORLD);
    
  // Printing only one result that will be used to create the image
    for (int yp=0; yp < p_count; ++yp) {
      for (int xp=0; xp < p_count; ++xp)
	std::cout << mset_r[yp*p_count + xp] << " ";
      std::cout << std::endl;
    }
  }
